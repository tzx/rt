...
Camera::Camera(int hsize, int vsize, float fov) : transform_(Matrix::identity_matrix(4)) {
  this->hsize_ = hsize;
  this->vsize_ = vsize;
  this->fov_ = fov;
...
Ray Camera::ray_for_pixel(int px, int py) const {
  ...
  Tuple pixel = this->transform().inverse() * Tuple::create_point(world_x, world_y, -1);
  Tuple origin = this->transform().inverse() * Tuple::create_point(0, 0, 0);
